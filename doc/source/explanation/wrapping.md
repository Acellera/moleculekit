...
For the task-focused recipe (parameters, common variations, and gotchas), see
[How to wrap trajectories](../how-to/wrap-trajectories.md). The rest of this
...
side (we never integrate those copies, which would be wasteful):
...
After running for a while it can look like this, which is normal given the
...
still interacting through the periodic copies, but with so many copies around it
is hard to interpret, which is why we prefer a single wrapped box:
...
a single atom or the average over a set of atoms (the `wrapsel` argument). The
right choice depends on the system and on personal taste.
...
- **Single protein in solvent.** Wrap so the protein sits in the middle of the
  ...
  others. There is no real vacuum, since the protruding piece occupies that
  space in a periodic image, but moleculekit keeps bonded molecules intact
  rather than breaking bonds across the boundary, so the picture stays
  one-sided:
