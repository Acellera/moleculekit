...
class SmallMol(object):
    ...
    def describe(self):
        ...
        Returns
        -------
        The values of the different descriptors
        The names of the different descriptors
        """
        ...
        for descname in descs:
            try:
                ...
                logger.warning(
                    f"Descriptor {descname} was not calculated for this small molecule due to {str(exception)}"
                )
                value = None  # "NaN"

            bin_name = f"DESC_{descname}"
            ans[bin_name] = value

        descr_values = ans.values()
        descr_headers = ans.keys()
        return descr_values, descr_headers
    ...
    def featurize(self, radius, num_bits):
        ...
        all_headers = ["smiles"]
        all_headers.extend([f"ECFC4_bit_{x}" for x in range(num_bits)])
        all_headers.extend([f"MACC_bit_{x}" for x in range(len(maccs))])
        all_headers.extend([f"Avalon_bit_{x}" for x in range(num_bits)])
        all_headers.extend(headers)

        return all_headers, features

    def strip_salts_and_detect_metals(
        self, metals_smart="[Mg,Ca,Zn,As,Mn,Al,Pd,Pt,Co,Ba,Cr,Cu,Ni,Ag,Fe,Hg,Cd,Gd,Na]"
    ):
