...
# moleculekit port: drop scipy.sparse dependency.  ``copy_if_needed`` is used
# with ``np.array(..., copy=copy_if_needed)`` -- in NumPy >=2 this is spelled
# ``None`` (copy only when required).
copy_if_needed = None

cimport numpy as np

from cpython.mem cimport PyMem_Malloc, PyMem_Free
# NOTE: ``libcpp.mutex`` with ``py_safe_call_once``/``py_safe_once_flag`` is
# only available in Cython >= 3.2.5.  To keep compatibility with older Cython
# we emulate the once-flag behaviour with a Python-level ``threading.Lock``
# inside the ``tree`` property below.
...
cdef class coo_entries:
    ...
    # moleculekit port: ``coo_array``/``coo_matrix``/``dok_*`` outputs require
    # ``scipy.sparse`` and have been removed.  Use ``ndarray()`` or ``dict()``
    # and construct any sparse matrix externally.
...
cdef class cKDTree:
    ...
    cdef:
        ...
        # moleculekit port: Python ``threading.Lock`` replaces the C++
        # ``py_safe_once_flag`` (Cython >= 3.2.5 only).
        object                   _tree_lock
    ...
    def sparse_distance_matrix(cKDTree self, cKDTree other,
        ...
        if output_type == 'dict':
            ...
            # moleculekit port: ``dok_matrix``/``coo_matrix``/``dok_array``/
            # ``coo_array`` require ``scipy.sparse`` and have been removed.
            raise ValueError(
                "Invalid output type -- only 'dict' and 'ndarray' are "
                "supported in the moleculekit port of cKDTree (scipy.sparse "
                "outputs have been dropped)."
            )
