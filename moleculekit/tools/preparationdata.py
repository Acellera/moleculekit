...
try:
    ...
    raise ImportError(
        "pdb2pqr not installed. To use the molecule preparation features please do `conda install htmd-pdb2pqr -c acellera`"
    )
...
class PreparationData:
    ...
    def __init__(self):
        ...
        self.data.pka_group_id = self.data.pka_group_id.astype(
            float
        )  # should be int, but NaN not allowed
    ...
    def _findRes(self, a_resname, a_resid, a_icode, a_chain):
        ...
        mask = (
            (self.data.chain == chain_pad)
            & (self.data.resid == a_resid)
        ...
        if not np.any(mask):
            ...
            assert (
                False
            ), "More than one resid matched: either duplicated chain-residue-icode, or internal error (please report if the latter)."
        return pos

    # Generic setter in the pandas table. Maybe we should use actual indices instead.
    def _set(self, residue, key, val, append=False):
        pos = self._findRes(
            residue.name, residue.resSeq, residue.iCode, residue.chainID
        )
        if not append:
    ...
    def _importPKAs(self, pkaCont):
        ...
        for i, grp in enumerate(self.propkaContainer.conformations["AVR"].groups):
            ...
            if grp.residue_type in [
                "N+",
                "C-",
            ]:  # Separate info about termini. See _findRes
                resname = grp.residue_type
                icode = "T"
                # forceAppend = True
            elif (
                grp.atom.sybyl_assigned
            ):  # A ligand - a hack to allow multiple groups overriding key. See _findRes
                icode = "L"
                # forceAppend = True
        ...
        if np.any(inSlabNotBuried):
            ...
            logger.warning(
                (
                    ...
                    + "exposed to the membrane ({:.1f}<z<{:.2f} and buried<{:.1f}%)."
                ).format(sum(inSlabNotBuried), -ht, ht, maxBuried)
            )
    ...
    def _listNonStandardResidues(self):
        ...
        for i, cr in self.data[changed].iterrows():
            if (
                cr.resname in ["N+", "C-"]
                or cr.protonation in ["WAT"]
                or type(cr.protonation) == float
            ):
                continue
            cl.append("{:s} ({:s})".format(prettyPrintResidue(cr), cr.protonation))
        if cl:
            logger.info(
                "The following residues are in a non-standard state: " + ", ".join(cl)
            )
    ...
    def _warnIfpKCloseTopH(self, ph, tol=1.0):
        ...
        if nd > 1:
            logger.warning(
                "Dubious protonation state: the pKa of {:d} residues is within {:.1f} units of pH {:.1f}.".format(
                    nd, tol, ph
                )
            )
            for i, dr in self.data[dubious].iterrows():
                ...
                logger.warning(
                    "Dubious protonation state:    {:s} (pKa={:5.2f})".format(
                        drs, dr.pKa
                    )
                )
    ...
    def _get_pka_plot(self, pH=7.4, figSizeX=10, dpk=1.0, font_size=12):
        ...
        grey_blue = LinearSegmentedColormap.from_list(
            "grey_blue", [neutral_grey, my_blue]
        )
        ...
        labels = [
            "{:s} {:s}:{:d}{:s}- {:s}".format(
                "(!)" if x.dubious else "", x.chain, x.resid, x.insertion, x.resname
            )
            for i, x in d.loc[titr].iterrows()
        ]
        pKas = d.pKa.loc[titr]
        restypes = [
            "neg" if x.resname in acidicResidues else "pos"
            for i, x in d.loc[titr].iterrows()
        ]
        ...
        ax.axvline(
            x=pH - dpk, linewidth=2, color="black", alpha=0.2, linestyle="dashed"
        )
        ax.axvline(
            x=pH + dpk, linewidth=2, color="black", alpha=0.2, linestyle="dashed"
        )
        ...
        for donor in protein.getAtoms():
            ...
            for acc in closeatoms:
                ...
                for donorh in donorhs:
                    # Do angle check
                    angle = getAngle(
                        acc.getCoords(), donor.getCoords(), donorh.getCoords()
                    )
    ...
    def getTerminiHbonds(self):
        ...
        for hb in hbl:
            ...
            if hb["donor"].name in ("O", "OXT") and hb["donor"].residue.isCterm == 1:
                ret["C"].append(hb)
            if (
                hb["acceptor"].name in ("O", "OXT")
                and hb["acceptor"].residue.isCterm == 1
            ):
                ret["C"].append(hb)
    ...
    def getTerminiBuriedness(self):
        ...
        ret = {
            "N": self.data.buried[self.data.resname == "N+"],
        ...
        for t in ["N", "C"]:
            ...
            for bf in tb[t]:
                if bf > buriedThreshold:
                    flag = True
                    logger.warning(
                        "Found {}-terminus {:.1f}% buried (> {:.1f}% threshold)".format(
                            t, bf, buriedThreshold
                        )
                    )
        return flag
    ...
    def reprepare(self):
        ...
        from moleculekit.tools.preparation import _buildResAndMol
        ...
        for index, oldResidue in enumerate(p.getResidues()):
            ...
            if sum(d_idx) != 1:
                logger.warning(
                    "Residue {:s} appears {:d} times in data table".format(
                        str(oldResidue), sum(d_idx)
                    )
                )
                continue
        ...
        newResData.data.loc[list_of_forced_protonations, "protonation"] = d.loc[
            list_of_forced_protonations, "forced_protonation"
        ]
...
class _TestPreparationData(unittest.TestCase):
